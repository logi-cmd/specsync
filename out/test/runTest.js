...
function testSpecParser() {
    console.log('\n📋 SpecParser 测试\n');
    const parser = new specParser_1.SpecParser();
    // 测试1: 基本解析
    const specContent = `---
title: 用户登录API
version: 1.0.0
...
function testCodeParser() {
    console.log('\n💻 CodeParser 测试\n');
    const parser = new codeParser_1.CodeParser();
    // 测试1: 基本解析
    const codeContent = `
...
`;
    const perfectReport = syncEngine.check(specParser.parse(perfectSpec), codeParser.parse(perfectCode));
...
`;
    const missingReport = syncEngine.check(specParser.parse(missingFieldSpec), codeParser.parse(missingFieldCode));
...
`;
    const typeReport = syncEngine.check(specParser.parse(typeMismatchSpec), codeParser.parse(typeMismatchCode));
...
`;
    const apiReport = syncEngine.check(specParser.parse(apiMissingSpec), codeParser.parse(apiMissingCode));
...
`;
    const constraintReport = syncEngine.check(specParser.parse(constraintSpec), codeParser.parse(constraintCode));
    // 密码约束应该被检测到（因为没有在注释中提到密码复杂度验证）
    const constraintIssue = constraintReport.inconsistencies.find(i => i.type === 'constraint_missing' && i.message.includes('password'));
...
`;
    const complexReport = syncEngine.check(specParser.parse(complexSpec), codeParser.parse(complexCode));
...
// 空文件
`;
    const emptyReport = syncEngine.check(specParser.parse(emptySpec), codeParser.parse(emptyCode));
...
`;
    const caseReport = syncEngine.check(specParser.parse(caseSpec), codeParser.parse(caseCode));
    // 类型应该匹配（忽略大小写）
