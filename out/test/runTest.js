...
function testTypeScriptAstParser() {
...
function testDecoratorAndFallbackMatching() {
...
function testDeepConstraintValidation() {
    const specParser = new specParser_1.SpecParser();
    const codeParser = new codeParser_1.CodeParser();
    const syncEngine = new syncEngine_1.SyncEngine();
    const spec = specParser.parse(`---
...
function testMissingConstraintDetection() {
    const specParser = new specParser_1.SpecParser();
    const codeParser = new codeParser_1.CodeParser();
    const syncEngine = new syncEngine_1.SyncEngine();
    const spec = specParser.parse(`---
...
`;
    ...
    assert.ok(report.inconsistencies.some(item => item.type === 'constraint_missing' && item.message.includes('必须包含字母和数字')));
}
function run() {
    testSpecParser();
